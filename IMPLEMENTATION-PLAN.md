...
| Sprint 26 | **COMPLETE** | CR18: Dynamic tool descriptions from Bullhorn /meta at startup — new `descriptions.py` module; `enrich_tool_descriptions()` runs in `main()` before `mcp.run()`; `BullhornMetadata.get_fields()` extended to retain picklist options; all 9 entity types covered; graceful fallback on auth/network failure — 389 tests passing |
