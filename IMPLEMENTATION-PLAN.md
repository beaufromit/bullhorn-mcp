...
**Note on `name` field:** Bullhorn requires `name` to be set explicitly via the API — it is not auto-computed from firstName/lastName as it is in the UI. CR26 enforces this: the MCP now always computes and injects `name = f"{firstName} {lastName}"` for both ClientContact and Candidate on create and on name-field updates. LLM-supplied `name` values are stripped and ignored.
...
**No critical issues. No moderate issues.** One minor:

- **m1: PRD.md still labels `get_contact` as "Planned (CR29)"** — The chore commit updated IMPLEMENTATION-PLAN.md but not the FR-18 status annotation in PRD.md. Fixed in post-review commit (PRD.md updated to "Implemented (CR29)").

563 tests passing, tagged v0.0.41.
...
**No critical issues.** One moderate:
...
- `test_update_candidate_name_recomputed` — passes `{"firstName": "John"}` only (one-sided); verifies `client.get` fetches current name fields and `name` is recomputed in the update payload. (Note: the CR31.md plan description incorrectly stated both firstName and lastName would be passed — those two claims are mutually exclusive; the test correctly implements the one-sided path that exercises `client.get`.)
...
**Risk:** Medium — the nested `notes.action` dot notation this sprint builds on is undocumented by Bullhorn. Mitigated by a live two-way canary in `scripts/smoke_read.py`; `/query/NoteEntity` (CR37 Part 3) is the fully-evidenced fallback design.
...
- **M3 fixed, then resolved differently on owner review (2026-07-29).** The finding was that the whole Part 6 deliverable was untracked (`?? .claude/skills/`). It was committed wholesale at v0.0.47; the owner then asked why a load-bearing operational check was living in an agent-context bundle at all. It should not have been. The resolution splits the directory by what the files actually are:
  - **`.claude/skills/` is gitignored** — 3,462 lines of agent working context that largely restates PRD.md, the CRx files and this plan. Local working context, not a repo deliverable.
  - **`.claude/commands/review.md` stays tracked** — it is the repo's only quality gate and the tracked `PROMPT_iterate.md` references it by path.
  - **The three runnable scripts moved to a tracked `scripts/` at repo root** — `smoke_read.py` (the live smoke check and two-way note canary), `meta_dump.py` (`/meta` field inventory for an entity), `measure_descriptions.py` (the enrichment token-cost measurement that CR34 and CR37 both required as an acceptance criterion). These are operational tooling that a person runs, not context an agent reads; nothing about them needed `.claude/`. Their `REPO_ROOT` path depth changed from `parents[4]` to `parents[1]` accordingly.
...
## Sprint 37: CR36 Housekeeping (verified loose ends from the 2026-07 handover audit) - COMPLETE (v0.0.48)
...
- [x] Local commit `fix: CR36 housekeeping ...`, then review cycle, push, and tag **v0.0.48** after the review cycle passes.
...
- **T37.7:** remote branch `origin/claude/fix-api-url-formatting-jn0DQ` (tip `64ad4b5`, confirmed merged into `origin/main`) **deleted 2026-09-23** with owner approval. **CASE_STUDY.md: accepted, history retained** (owner, 2026-09-23). The owner's requirement is that the file is not in the repo going forward, and that already holds: it was reverted in `c570499` (Jan 2026), is absent from the tree and index, and `.gitignore:41` blocks it from being re-added. A history rewrite was rejected because it cannot remove the content: this repo is a public GitHub fork of `osherai/bullhorn-mcp-python`, whose public history still holds `cdca5f8`, and fork networks share objects, so the SHA stays reachable. The cost would have been rewriting 135 of 137 commits and all 47 tags, plus a force-push. T37.7 is closed.
...
- **Cycle 1 (reviewed `7142cbb` + `bf91c18` as `git diff HEAD~2`): clean.** 0 CRITICAL, 0 MODERATE. Six minors logged, not fixed per PROMPT_iterate constraint 1:
  - **m1:** a `resolve_owner` quote-rejection `ValueError` surfaces under the `owner_not_found` / `user_not_found` error code, so the caller sees "not found" for what is really a rejected input. The comment at `server.py:1415` is now incomplete.
  - **m2:** the `resolve_owner` docstring `Raises:` section does not mention the quote rejection.
  - **m3:** the reject-only choice hard-locks names and emails containing an apostrophe (e.g. O'Brien), a real population on an Irish tenant. Any caller whose Entra email contains an apostrophe cannot use the write paths that need `resolve_caller`. Not a regression (the old query was malformed for these inputs anyway). Candidate for a future CR: escape instead of reject.
  - **m4:** two test docstrings call the `/query` WHERE clause "Lucene".
  - **m5:** the sprint status row still said T37.7 was pending after `bf91c18` closed it. Fixed by this update.
  - **m6:** the README "Supported write targets" and "Features" sections are still stale (out of T37.6 scope, already noted in the build notes above).
- Tagged v0.0.48 at 719 tests (2026-09-23).
- **Learning:** the reviewer flagged m3 as the most consequential minor. Recommend the owner consider it for a CR.
...
- **T39.5 live acceptance PASSED (owner-approved, 2026-09-23, stdio mode, so the caller did not resolve and the record-owner fallback ran):**
  1. `add_note("JobOrder", 51437, "Claude note", ...)` created note **2653738**: `personReference` = CorporateUser 142235 (job owner Beau Warren), `jobOrders` = [51437], no `jobOrder` TO_ONE. `get_notes_for_entity("JobOrder", 51437)` returns it.
  2. 2653738 does **not** appear under ClientContact 145635.
  3. `add_note("ClientCorporation", 10666, ...)` returned `company_notes_live_on_contacts` with 22 contacts and wrote nothing. `get_notes_for_entity("ClientCorporation", 10666)` returned notes (no 500; `total` None, `has_more` true via the page-count heuristic). `search_notes("Claude", entity_filter={ClientCorporation, 10666})` returned 2653704 (no 500).
