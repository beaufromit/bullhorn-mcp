...
- **M3 partly fixed, then deliberately narrowed by owner decision (2026-07-29).** The finding was that the whole Part 6 deliverable was untracked (`?? .claude/skills/`). It was committed at v0.0.47, then the owner ruled that `.claude/skills/` is local working context rather than a repo deliverable; it is now in `.gitignore` and untracked again. `.claude/commands/` stays tracked, since it holds the shared review protocol. **Standing consequence:** the two-way live canary in `.claude/skills/bullhorn-mcp-live-api-method/scripts/smoke_read.py` — CR37 Change 1's only stated mitigation for the undocumented `notes.action` dot notation — does not survive a clean checkout, and a future reviewer will not see it in any diff. If that syntax is ever withdrawn by Bullhorn, the fallback design is `/query/NoteEntity` (CR37 Part 3, fully evidenced). Promoting the canary into a tracked path such as `scripts/` would close this; it has not been done. The other half of M3 — the missing Sprint 36 section — is this section, and stands.
